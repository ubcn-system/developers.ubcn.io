...
<br/>
<strong style={"color: green"}>GET</strong> /user/search/terminalId/`{terminalId}`
<Frame>
```ts
    //유비씨엔(주)에서 제공하는 단말기 ID를 기준으로 사용자 계정 정보를 조회합니다.
    const res = await API.get("/user/search/terminalId/{terminalId}");
```
</Frame>
<Frame>
    ### Request Schema
...
<br/>
<strong style={"color: green"}>GET</strong> /search/`{couponNo}`/user/`{id}`
<Frame>
    ```ts
    //쿠폰 번호로 쿠폰의 현재 상태를 조회합니다.
    const res = await API.get("/search/{couponNo}/user/{id}");
    ```
</Frame>
<Frame>
    ### Request Schema
